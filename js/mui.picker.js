...
(function($, window, document, undefined) {
	...
	Picker.prototype.calcElementItemPostion = function(andGenerateItms) {
		...
		self.elementItems.forEach(function(item) {
			...
			if (andGenerateItms) {
				var dataItem = {};
				dataItem.areaName = item.innerHTML || '';
				dataItem.areaId = item.getAttribute('data-value') || dataItem.areaName;
	...
	Picker.prototype.setItems = function(items) {
		...
		self.items.forEach(function(item) {
			if (item !== null && item !== undefined) {
				buffer.push('<li>' + (item.areaName || item) + '</li>');
			}
		});
	...
	Picker.prototype.getSelectedValue = function() {
		var self = this;
		return (self.items[self.getSelectedIndex()] || {}).areaId;
	};

	Picker.prototype.getSelectedText = function() {
		var self = this;
		return (self.items[self.getSelectedIndex()] || {}).areaName;
	};
